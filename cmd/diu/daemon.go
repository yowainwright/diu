...
func restartDaemon(cmd *command, args []string) error {
	...
	if err := stopDaemonInternal(config); err != nil {
		return err
	}
	time.Sleep(time.Second)
	return startDaemonInternal(config)
}
...
// startDaemonInternal is used by restartDaemon to avoid infinite recursion
func startDaemonInternal(config *core.Config) error {
	// Check if already running
	if defaultDaemonChecker.IsRunning(config) {
		fmt.Println(infoStyle.Render("DIU daemon is already running"))
		return nil
...
// stopDaemonInternal is used by restartDaemon to avoid infinite recursion
func stopDaemonInternal(config *core.Config) error {
	if !defaultDaemonChecker.IsRunning(config) {
		fmt.Println(infoStyle.Render("DIU daemon is not running"))
		return nil
	}
