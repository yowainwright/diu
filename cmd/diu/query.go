...
func openExecutionStore() (storage.Storage, error) {
...
func statsOptionsFromCommand(cmd *command) statsCommandOptions {
	...
	return statsCommandOptions{query: opts, daily: daily, weekly: weekly, tool: toolFilter, top: top}
}
...
func printStatsHeading(out *dx.Out, options statsCommandOptions) {
	...
	out.Println(out.DataStyle(dx.Accent, title))
	out.Println()
}
...
func printMostActiveDay(out *dx.Out, stats *core.StorageStatistics, options statsCommandOptions) {
	hasMostActiveDay := stats.MostActiveDay != ""
	showingSummary := options.daily || options.weekly
	showMostActiveDay := hasMostActiveDay && !showingSummary
	if showMostActiveDay {
		out.Printf("%s %s\n",
