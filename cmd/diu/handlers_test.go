...
func TestUninstallProjectRemovesSetupArtifactsAndPreservesData(t *testing.T) {
	...
	if err := config.EnsureDirectories(); err != nil {
		t.Fatalf("Failed to create DIU directories: %v", err)
	}
...
DIU_TOOL="homebrew"
`
	writeExecutableForTest(t, wrapperPath, wrapper)
	unrelatedPath := filepath.Join(config.Monitoring.Process.WrapperDir, "notes")
	writeExecutableForTest(t, unrelatedPath, "#!/bin/bash\necho '# Generated by DIU'\n")

	pathLine := fmt.Sprintf("export PATH=\"%s:$PATH\"", config.Monitoring.Process.WrapperDir)
	zshrc := filepath.Join(homeDir, ".zshrc")
	zshContent := "# existing\n\n# DIU path configuration\n" + pathLine + "\nalias ll='ls'\n"
	if err := os.WriteFile(zshrc, []byte(zshContent), core.PrivateFileMode); err != nil {
		t.Fatalf("Failed to write shell config: %v", err)
	}

	data := []byte(`{"executions":[]}`)
	if err := os.WriteFile(config.Storage.JSONFile, data, core.PrivateFileMode); err != nil {
		t.Fatalf("Failed to write usage data: %v", err)
	}

	output := captureStdout(t, func() {
		if err := uninstallProject(&command{}, nil); err != nil {
			t.Fatalf("uninstallProject failed: %v", err)
		}
	})

	if _, err := os.Stat(wrapperPath); !os.IsNotExist(err) {
		t.Fatalf("Expected generated wrapper removal, stat err=%v", err)
	}
	if _, err := os.Stat(unrelatedPath); err != nil {
		t.Fatalf("Expected unrelated wrapper directory file to remain: %v", err)
	}
	updatedZsh, err := os.ReadFile(zshrc)
	if err != nil {
		t.Fatalf("Failed to read shell config: %v", err)
	}
	if string(updatedZsh) != "# existing\nalias ll='ls'\n" {
		t.Fatalf("Unexpected shell config after uninstall: %q", updatedZsh)
	}
	if _, err := os.Stat(config.Storage.JSONFile); err != nil {
		t.Fatalf("Expected usage data to remain: %v", err)
	}
...
DIU_TOOL="homebrew"
`
	writeExecutableForTest(t, wrapperPath, legacyWrapper)

	if err := uninstallProject(&command{}, nil); err != nil {
		t.Fatalf("uninstallProject failed: %v", err)
	}
	if _, err := os.Stat(wrapperPath); !os.IsNotExist(err) {
		t.Fatalf("Expected legacy wrapper removal, stat err=%v", err)
	}
}
...
func TestUninstallProjectRemovesSupportedShellEntries(t *testing.T) {
	...
	pathMarker := core.ShellPathMarker + "\n"
	...
	bashContent := "before\n" + pathMarker + posixLine + "\nafter\n"
	zshContent := "before\n\n" + pathMarker + posixLine + "\nafter\n"
	fishContent := pathMarker + fishLine + "\nafter\n"
	files := []struct {
		path    string
		content string
		want    string
	}{
		{path: bashPath, content: bashContent, want: "before\nafter\n"},
		{path: zshPath, content: zshContent, want: "before\nafter\n"},
		{path: fishPath, content: fishContent, want: "after\n"},
	}
	for _, file := range files {
		if err := os.WriteFile(file.path, []byte(file.content), core.PrivateFileMode); err != nil {
	...
	for _, file := range files {
		...
		if string(content) != file.want {
			t.Fatalf("Unexpected content in %s: %q", file.path, content)
		}
	}
}
