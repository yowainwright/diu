...
The container process limit protects the test environment. Production wrappers have no global recorder concurrency cap, and these tests do not establish a safe maximum command rate.
...
| Recording | Real fallback history contains the original execution and manager metadata; slow recorders do not retain pipes; locked storage drops events; recorder children do not recursively record; helper commands bypass PATH. |
