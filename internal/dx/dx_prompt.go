...
func (p *Prompter) Confirm(message string, isDefaultConfirmed bool) (bool, error) {
	hint := " [y/N]"
	if isDefaultConfirmed {
		hint = " [Y/n]"
	}
	p.writePrompt(message, hint)
	value, err := p.readLine()
	return parseConfirmation(value, err, isDefaultConfirmed)
}
...
func parseConfirmation(value string, err error, isDefaultConfirmed bool) (bool, error) {
	if err != nil {
		return false, err
	}
	switch strings.ToLower(value) {
	case "":
		return isDefaultConfirmed, nil
	case "y", "yes":
		return true, nil
	case "n", "no":
		return false, nil
