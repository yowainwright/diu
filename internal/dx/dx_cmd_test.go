...
func nestedHelpCommand() (*dx.Command, *bytes.Buffer) {
	...
	child.AddCommand(&dx.Command{Use: "hidden", IsHidden: true})
	root.AddCommand(child)
	return root, &stdout
}
...
func separatedValueFlags(state *separatedFlagState) *dx.FlagSet {
	...
	yesDefault := false
	flags.BoolVarP(&state.hasYes, "yes", "y", yesDefault, "yes")
	return flags
}
...
func assertSeparatedFlags(t *testing.T, state *separatedFlagState, remaining []string) {
	...
	gotTool := state.tool == "npm"
	gotLimit := state.limit == 5
	gotExpectedFlags := gotTool && gotLimit && state.hasYes
	if !gotExpectedFlags {
		t.Fatalf("flags = %q, %d, %v", state.tool, state.limit, state.hasYes)
	}
